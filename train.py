...
from models import LightCNN, AudioResNet, AudioMobileNetV2, AudioAlexNet
...
def train(model_name, epochs=10, batch_size=64, lr=0.001):
    ...
    if model_name == 'light':
        ...
        model = AudioMobileNetV2(num_classes=10).to(device)
    elif model_name == 'alexnet':
        model = AudioAlexNet(num_classes=10).to(device)
    else:
    ...
    best_val_acc = 0.0
    csv_path = os.path.join(LOG_DIR, f"training_history_{model_name}.csv")
        ...
        for epoch in range(epochs):
            ...
            with torch.no_grad():
                ...
                for audio, labels in val_loader:
                    ...
                    outputs = model(audio)
                    loss = criterion(outputs, labels)
                    val_loss_sum += loss.item() * audio.size(0)
                    _, predicted = outputs.max(1)
            ...
            writer.writerow([epoch + 1, f"{epoch_loss:.6f}", f"{val_loss:.6f}", f"{val_acc:.4f}"])
            csv_file.flush()
            print(f"Epoca [{epoch+1}/{epochs}] - Loss Train: {epoch_loss:.4f} - Acc Train: {epoch_acc:.2f}% | Loss Val: {val_loss:.4f} | Acc Val: {val_acc:.2f}%")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Train Speech Commands con diversi modelli")
